...
**Status:** **shipped, both languages, pre-publish-hardened.** All 11 implementation steps complete (2026-09-11) — Go reference at `RSA4096/` and TypeScript port at `ts/src/rsa4096/` (`@ouronet/dalos-crypto/rsa4096`), both graduated from the `research/rsa4096-poc/` prototype into the real package, with a frozen, CI-pinned test-vector corpus (`testvectors/v2_rsa4096.json`) and full cross-language byte-identity (`ts/tests/rsa4096/rsa4096.test.ts` — every field, including exact candidate-attempt counts). Same day, three more changes before publish (§11): Miller-Rabin bumped 64→100 rounds, a real purely-observational progress-reporting API (plus a TS event-loop-yielding async variant) so a UI can render a live, mathematically-honest progress bar, and the seed length generalized beyond DALOS Genesis's 1600 bits (APOLLO's 1024 bits confirmed working). See §9 (steps 1-9), §10 (steps 10-11), and §11 (round count + progress API + seed-length generalization) at the end of this document. Validated against **real, independent code** throughout — `arweave-core`'s actual `importKeyfile`/`addressOf`, and Node's native WebCrypto RSA-PSS sign/verify — not just internal self-checks. Remaining before production use: npm publish, wiring into Codex's actual key-generation flow, and the live-gateway transaction test (deliberately not attempted autonomously at any point — needs a funded wallet and a human present; this is the user's own explicit next step).
