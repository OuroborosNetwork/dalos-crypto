...
**Status:** working prototype, steps 1-9 of the implementation plan complete and independently validated (2026-09-11, `research/rsa4096-poc/`) — see §9 "Implementation Log" at the end of this document. §5.0 resolved by direct Go stdlib source tracing (§8): `crypto/rsa.GenerateKey` is permanently unusable for this purpose; the prime search is built from scratch. The prototype end-to-end pipeline (seed → deterministic stream → prime search → key assembly → JWK) is proven correct against **real, independent code** — `arweave-core`'s actual `importKeyfile`/`addressOf`, and Node's native WebCrypto RSA-PSS sign/verify — not just internal self-checks. Remaining: steps 10-11 (graduate out of the scratch module into a real `RSA4096/` package with frozen corpus + CI pin; TypeScript port) and the live-gateway half of step 8 (deliberately not attempted autonomously — needs a funded wallet and a human present).
