...
**One seed, many independent Arweave addresses (not just one).** `GenerateFromBitStringAtIndex(seedBitString, index, onProgress)` derives address `#index` from the same seed bitstring — RSA has no BIP-32-style non-hardened child-key trick, so every indexed address is a genuinely independent full keypair, but the derivation only needs one extra domain-separated Blake3 hash (`DALOS-gen1/RSA4096Index/v1`) to produce a fresh, independent seed per index before running the unmodified prime search. **`index == 0` is structurally guaranteed byte-identical, forever, to the plain `GenerateFromBitString` call** — this can never silently change any address already in use. Any index is directly reachable without generating the ones before it — a pure function of `(seed, index)`, not a "must walk forward from 0" model. `GenerateBatchFromBitString(seedBitString, startIndex, count, onProgress, onResult)` adds sequential orchestration for "the first N addresses" (or any range): one combined progress readout across the whole batch, results delivered incrementally as each one completes, and a settled answer for partial failure (results completed so far are returned alongside the error, never discarded). `GenerateFromBitStringAtRanges(seedBitString, ranges []IndexRange, onProgress, onResult)` generalizes that single contiguous range to an arbitrary LIST of inclusive ranges (e.g. `1-100`, `134-167`, `234-777`) in one deterministic call — deduplicated, sorted ascending, with index 0 **always** included regardless of whether any given range covers it. A separate function from `GenerateBatchFromBitString`, not a modification of it, so that function's exact published contract never changes for existing callers. Frozen corpus: `testvectors/v3_rsa4096_indexed.json` (ranges reuses its `rsa4096-idx-01/02/03` vectors as its cross-check too — pure orchestration over indexed generation needs no new corpus).
